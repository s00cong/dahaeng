...
import { Bell, BellOff, PlaneTakeoff, CheckCircle2, TrendingDown } from 'lucide-react';
    ...
    <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <div className="flex items-center gap-2 mb-4">
        ...
        <div>
          ...
          <p className="text-xs text-slate-400">{cityName} 목표가 이하 도달 시 알림</p>
        </div>
        {subscription && (
      ...
      {subscription && (
        <div className="mb-4 rounded-xl bg-slate-50 border border-slate-100 p-3 space-y-2">
          <div className="flex items-center justify-between text-sm">
            ...
            <span className="font-semibold text-slate-800">{formatKRW(subscription.thresholdPrice)}</span>
          </div>
              ...
              {subscription.lastNotifiedAt && (
                <div className="flex items-center justify-between text-xs">
                  ...
                  <span className="text-slate-400">
                    {dayjs(subscription.lastNotifiedAt).format('YYYY.MM.DD HH:mm')}
                  </span>
                </div>
              )}
            </>
