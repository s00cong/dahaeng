...
export function FlightTab({ city }: FlightTabProps) {
  ...
  const { data: summary, isLoading: summaryLoading } = useCitySummary(
    cityId,
    selectedYearMonth,
  );

  // 현재 월 및 다음 월 데이터 가져오기 (월간 경계 가격 조회용)
  const nextMonthTab = MONTH_TABS.find(
    (_, i) => i > 0 && MONTH_TABS[i - 1].yearMonth === selectedYearMonth,
  );
  const nextYearMonth = nextMonthTab?.yearMonth;

  const { data: calendar, isLoading: calendarLoading } = useFlightCalendar(
    cityId,
    selectedYearMonth,
  );
  ...
  const isPeakSeason =
    summary?.peak_season_months.includes(selectedMonth) ?? false;
  const isOffSeason =
    summary?.off_season_months.includes(selectedMonth) ?? false;
    ...
    <div className="flex flex-col gap-6 p-6 h-full overflow-y-auto relative">
        ...
        {MONTH_TABS.map((tab) => {
          ...
          return (
            <button
              ...
              onClick={() => {
                setSelectedYearMonth(tab.yearMonth);
                setActiveDay(null);
                setDeparture(null);
                setReturnDate(null);
              }}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 min-h-0 items-start">
        ...
        <div ref={rightColRef} className="flex flex-col gap-4 min-w-0">
          ...
          <div className="flex-1 relative overflow-hidden rounded-xl mb-6 border border-border/50 bg-card/50 min-h-[380px] flex flex-col">
      ...
      <div className="bg-card border border-border rounded-xl px-6 py-4 flex items-center justify-between shadow-sm">
        <div className="flex items-center gap-3 flex-1 justify-center">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-base font-black text-foreground">
              {summary.avg_flight_price.toLocaleString()}원
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 flex-1 justify-center">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-base font-black text-foreground">
              {summary.avg_hotel_price.toLocaleString()}원
            </span>
          </div>
        </div>
        <div className="w-px h-8 bg-border mx-2" />
...
function FlightCalendarGrid({
  ...
  const { cells, outboundMap, inboundMap, fullInboundMap, today, minPrice } =
    useMemo(() => {
      ...
      // 현재 월 데이터 맵핑
      if (calendar) {
        for (const d of calendar.outbound_daily_prices)
          outMap.set(dayjs(d.date).date(), d);
      ...
      for (let day = 1; day <= days; day++) {
        ...
        if (priceView === "outbound") {
          ...
          if (travelDuration !== null) {
            ...
            const inboundDateStr = dayjs(currentFullDate)
              .add(travelDuration, "day")
              .format("YYYY-MM-DD");
      ...
      return {
        ...
        daysInMonth: days,
        minPrice:
          currentMinPrice === Number.POSITIVE_INFINITY ? null : currentMinPrice,
      };
    }, [
      calendar,
      nextCalendar,
      ...
      <div className="flex flex-col mb-3 gap-2 px-1">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            {isPeakSeason && (
              <Badge className="text-[10px] px-1.5 py-0 h-5 bg-red-100 text-red-600 border-0 font-bold">
                성수기
              </Badge>
            )}
            {isOffSeason && (
              <Badge className="text-[10px] px-1.5 py-0 h-5 bg-blue-100 text-blue-600 border-0 font-bold">
                비성수기
              </Badge>
            )}
          </div>
      ...
      <div className="bg-card border border-border rounded-xl overflow-hidden shadow-sm flex-1">
        ...
        >
          {cells.map((day, idx) => {
            ...
            const hasData =
              (priceView === "outbound" && out) ||
              (priceView === "inbound" && inn) ||
              priceView === "total";
            const dow = idx % 7;
            const isPast = dayjs(currentFullDate).isBefore(
              dayjs().startOf("day"),
            );
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  isPast &&
                    "cursor-not-allowed bg-[repeating-linear-gradient(135deg,transparent,transparent_4px,rgba(0,0,0,0.04)_4px,rgba(0,0,0,0.04)_5px)]",
                  !isPast &&
                    hasData &&
                  ...
                  !isPast &&
                    ...
                    "bg-blue-600 text-white z-10 shadow-md scale-105 rounded-md",
                  !isPast &&
                    !isDeparture &&
                    !isReturn &&
                    isSelected &&
              ...
              >
                ...
                {!isPast && price !== undefined ? (
                  <span
                    className={cn(
                      ...
                      isDeparture || isReturn
                        ? "text-blue-50"
                        : isMinPrice
                          ? "text-orange-600 underline decoration-2"
                          : "text-blue-600",
                    )}
                  >
                    {formatWan(price)}
                  ...
                  <span className="text-[10px] leading-none text-transparent select-none">
                    -
                  </span>
                )}

                {!isPast && isDeparture && (
                  <span className="absolute -top-1 left-1/2 -translate-x-1/2 text-[8px] font-black bg-white text-blue-600 px-1 rounded border border-blue-600">
                    출발
                  </span>
                )}
                {!isPast && isReturn && (
...
function DateHistoryPanel({
  ...
  const displayDay =
    ...
    (departure
      ? {
          day: dayjs(departure.date).date(),
          outbound: departure.entry,
          inbound: undefined,
        }
      : null);
  ...
  const totalPriceTrend = useMemo(() => {
    ...
    const dateMap = new Map<
      string,
      { label: string; outPrice?: number; inPrice?: number }
    >();

    for (const h of outHistory) {
      const existing = dateMap.get(h.collected_date) ?? {};
      dateMap.set(h.collected_date, {
        ...existing,
        label: h.label,
        outPrice: h.price,
      });
        ...
        >
            ...
            <div className="flex flex-col">
              <p className="text-2xl font-black text-foreground tracking-tight">
                {isRangeComplete
                  ...
                  : dayjs(
                      `${yearMonth}-${String(displayDay?.day).padStart(2, "0")}`,
                    ).format("M월 D일 (ddd)")}
              </p>
          ...
          <div className="grid grid-cols-2 gap-3">
            <HistoryCard
              ...
              subLabel={
                isRangeComplete
                  ? dayjs(departure.date).format("M/D")
                  : undefined
              }
            />
            <HistoryCard
              ...
              subLabel={
                isRangeComplete
                  ? dayjs(returnDate.date).format("M/D")
                  : undefined
              }
            />
          </div>
          ...
          {(isRangeComplete ||
            (displayDay?.outbound && displayDay?.inbound)) && (
            <div className="flex items-center justify-between rounded-xl px-5 py-3.5 bg-slate-900 text-white dark:bg-slate-100 dark:text-slate-900 shadow-sm">
              ...
              <span className="text-2xl font-black">
                {isRangeComplete
                  ...
                  : (
                      (displayDay?.outbound?.price || 0) +
                      (displayDay?.inbound?.price || 0)
                    ).toLocaleString()}
                원
              </span>
            </div>
          ...
          {!isRangeComplete && departure && !activeDay && (
            <div className="text-center py-2 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-100 dark:border-blue-800">
              <p className="text-[10px] font-bold text-blue-600 dark:text-blue-400 animate-pulse">
                귀국 날짜를 선택해주세요
              </p>
            </div>
          )}
    ...
    <div className="rounded-xl p-3.5 flex flex-col gap-2.5 border border-border bg-card shadow-sm">
      ...
      {oldest && (
        <div className="flex items-center justify-between text-[10px] text-muted-foreground border-t border-border pt-2">
          <span>
            {oldest.label} {oldest.price.toLocaleString()}원
          </span>
          <span
            className={cn(
              "font-bold",
...
function TrendSection({
  ...
  const { chartData, domain, cheapestMonth } = useMemo(() => {
    ...
    const cheap = trend.trend_data.reduce((a, b) =>
      a.avg_flight_price < b.avg_flight_price ? a : b,
    );
    return {
      chartData: data,
  ...
  if (isLoading) return <Skeleton className="w-full h-80 rounded-xl" />;
  if (!trend || trend.trend_data.length === 0)
    return <EmptyState message="추이 정보가 없습니다." />;
      ...
      <div className="bg-card border border-border rounded-xl p-4 shadow-sm flex-1 flex flex-col justify-center">
        {cheapestMonth && trendView === "flight" && (
          <div className="mb-3 text-[11px] font-bold text-blue-600 bg-blue-50 px-2.5 py-1 rounded-full self-start">
            최저: {dayjs(cheapestMonth.year_month + "-01").format("M월")} (
            {cheapestMonth.avg_flight_price.toLocaleString()}원)
          </div>
        )}
