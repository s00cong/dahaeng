...
import { Globe, TrendingUp, TrendingDown, ChevronDown, Search } from 'lucide-react';
...
// ─── 국가 선택 드롭다운 ───────────────────────────────────────────
interface CountryOption { id: number; name: string; dailyBudget: number }
...
// ─── 메인 컴포넌트 ────────────────────────────────────────────────
export function CostCountryCompareSection() {
  const [baseId, setBaseId] = useState<number>(SEOUL_COUNTRY_ID);
  const [targetId, setTargetId] = useState<number | null>(null);
  const [compareIds, setCompareIds] = useState<{ base: number; target: number } | null>(null);

  // 전체 국가 목록 (CONTINENT 타입으로 전체 국가 조회)
  ...
  const options: CountryOption[] = (countryList ?? []).map((c) => ({
    id: c.id,
    name: c.name,
  ...
  const { data: compareData, isLoading } = useQuery({
    queryKey: ['cost', 'compare', 'country', compareIds?.base, compareIds?.target],
    queryFn: () => costApi.getCostCompare('COUNTRY', compareIds!.base, compareIds!.target),
  ...
  return (
    <section aria-label="국가 물가 비교">
      <div className="flex items-center gap-2 mb-5">
        <Globe className="size-5 text-violet-500" />
        <h2 className="text-xl font-bold text-foreground">국가 물가 비교</h2>
      </div>
      ...
      <div className="bg-card rounded-2xl border border-border/60 shadow-sm p-5">
        ...
        <div className="flex flex-col sm:flex-row items-end gap-3">
          ...
          <Button
            ...
            className="h-11 px-6 bg-violet-500 hover:bg-violet-400 text-white shrink-0 mb-0.5"
          >
            비교하기
          </Button>
        </div>
        ...
        {!compareIds && !isLoading && (
          <div className="mt-6 py-10 text-center text-sm text-muted-foreground">
            두 국가를 선택하고 <strong>비교하기</strong>를 눌러주세요
          </div>
        )}
      </div>
    </section>
