...
# ─── 기존 CSV에 이어붙이기 위한 설정 ───────────────────────────────────────
EXISTING_CSV = r"C:\Users\SSAFY\workspace\구글플라이트 크롤링\v5_공항id에 따른 검색\explore_prices_live_20260306_132558.csv"
...
async def scrape_google_explore():
    ...
    if os.path.exists(EXISTING_CSV):
        ...
        print(f"[Init] Loaded {len(all_results)} existing rows from {EXISTING_CSV}")
    else:
        all_results = {}
        print(f"[Init] CSV not found, starting fresh: {EXISTING_CSV}")

    lock = asyncio.Lock()
    incremental_filename = EXISTING_CSV  # 동일 파일에 저장

    async with async_playwright() as p:
        browser = await p.chromium.launch(channel="chrome", headless=False, slow_mo=50)
        context = await browser.new_context(viewport={'width': 1920, 'height': 1080})

        print("\n[Explore] >>> Starting append scan for missing airports")
